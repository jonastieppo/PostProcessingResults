...
def CalculateCalculateEngineeringConstants(ComplianceMatrix):
    ...
    v21 = -E2*df.iloc[0,1]
    v31 = -E3*df.iloc[0,2]
    eta1_12 = G12*df.iloc[0,5]
    eta_23 = G23*df.iloc[0,3]
    eta1_31 = G31*df.iloc[0,4]

    v12 = -E1*df.iloc[1,0]
    v32 = -E3*df.iloc[1,2]
    eta2_12 = G12*df.iloc[1,5]
    eta2_23 = G23*df.iloc[1,3]
    eta2_31 = G31*df.iloc[1,4]

    v13 = -E1*df.iloc[2,0]
    v23 = -E2*df.iloc[2,1]
    eta3_12 = G12*df.iloc[2,5]
    eta3_23 = G23*df.iloc[2,3]
    eta3_31 = G31*df.iloc[2,4]

    eta12_1 = E1*df.iloc[5,0]
    eta12_2 = E2*df.iloc[5,1]
    eta12_3 = E3*df.iloc[5,2]
    mi12_23 = G23*df.iloc[5,3]
    mi12_31 = G31*df.iloc[5,4]

    eta23_1 = E1*df.iloc[3,0]
    eta23_2 = E2*df.iloc[3,1]
    eta23_3 = E3*df.iloc[3,2]
    mi23_12 = G12*df.iloc[3,5]
    mi23_31 = G23*df.iloc[3,4]

    eta31_1 = E1*df.iloc[4,0]
    eta31_2 = E2*df.iloc[4,1]
    eta31_3 = E3*df.iloc[4,2]
    mu31_12 = G12*df.iloc[4,5]
    mi31_23 = G23*df.iloc[4,3]
...
class PostProcessing():
    ...
    def ExportStiffness(self,folder=f'{TellsCurrentDirectory()}',name="stiffness"):
        '''
        Just a method to print the stiffness
        '''
        DataFrame = self.Stiffness
        DataFrame.to_csv(fr'{folder}\{name}.out',sep='\t', header = None, index = False)
