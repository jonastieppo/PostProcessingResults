...
def compare_to_experiments(FiberDict,MatrixDict,h):
        '''
        Method to analyse the E-glass/Epoxy Plain Weave
        '''

        homogenized_lamina = Homogeneous_lamina(FiberDict,MatrixDict,vf=FiberDict['vf'])
        Qbar_0 = Qbar(0,EngineeringConstants=homogenized_lamina)
        Qbar_90 = Qbar(90,EngineeringConstants=homogenized_lamina)
        step=h/4
        z_info1 = z_info(-h/2,-h/2+step)
        z_info2 = z_info(-h/2+step,0)
        z_info3 = z_info(0,h/2-step)
        z_info4 = z_info(h/2-step,h/2)
